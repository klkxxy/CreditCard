...
- (void)viewDidLoad {
    ...
    self.showInView.backgroundColor = MX_GRAY_COLOR;
...
- (void)submitToServer{
    ...
    countImgNum = [self.bigImageArray count];
    for (UIImage *image in self.bigImageArray) {
        NSTimeInterval interval = [[NSDate date] timeIntervalSince1970] * 1000;
        
        NSFileManager *fileManager = [NSFileManager defaultManager];
        NSString *rootPath = NSSearchPathForDirectoriesInDomains(NSDocumentDirectory, NSUserDomainMask, YES)[0];
        NSString *filePath = [NSString stringWithFormat:@"%@/%f.jpg",rootPath,interval];
        NSData *imageData = UIImageJPEGRepresentation(image, 0.25);
        [fileManager createFileAtPath:filePath contents:imageData attributes:NULL];
        if ([fileManager fileExistsAtPath:filePath]) {
            [MXPublicRequstClass uploadImg:filePath success:^(id result) {
                ...
                if (countImgNum == 0) {
                    ...
                    NSDictionary *dicParams = [NSDictionary dictionaryWithObjectsAndKeys:self.noteTextView.text,@"messages_info",[MXUserMessageTool message].user_id,@"user_id",upload_imgNameArray,@"picture_arr",upload_imgSizeArray,@"picture_size", nil];
